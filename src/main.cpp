...
#if defined(NDEBUG)
# error "TUcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6799852c6028114c608646d4bc9a7e19d4f64a993cbdd6f0efabec967825bf87");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TUcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TUcoin Signed Message:\n";
...
{
    // TUcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // TUcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // TUcoin: 5 minutes
static const int64 nTargetSpacing = 1 *  24 * 60 * 60; // TUcoin: 1 day
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TUcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // TUcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6799852c6028114c608646d4bc9a7e19d4f64a993cbdd6f0efabec967825bf87");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is TUcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x18dc7d5b3035b8acec184e0c90235f20e5ad310a93c888ad16e2625087363159"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // TUcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TUcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TUcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TUcoinMiner(CWallet *pwallet)
{
    printf("TUcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tucoin-miner");
    ...
    try { loop {
        ...
        printf("Running TUcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TUcoinMiner, pwallet));
}
